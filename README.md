...
* A possible expansion is the utilization of two levels of classifiers as done in $^{[2]}$, where the first selects the two most probable classes, and the second more accurately differentiates between them. 
* Another expansion is to not use approximation $(3)$, and to include the gradients of all the convolution layers of the architecture in the lensing equation
